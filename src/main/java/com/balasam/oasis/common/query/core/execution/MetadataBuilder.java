...
public class MetadataBuilder {
    ...
    private Map<String, QueryResult.QueryMetadata.AttributeMetadata> buildAttributesMetadata() {
        ...
        for (Map.Entry<String, AttributeDef> entry : definition.getAttributes().entrySet()) {
            ...
            QueryResult.QueryMetadata.FilterMetadata filterMetadata = null;
            
            if (activeFilter != null) {
                filterMetadata = QueryResult.QueryMetadata.FilterMetadata.builder()
                    .attribute(attrName)
                    .operator(activeFilter.getOperator().name())
                    .value(activeFilter.getValue())
                    .value2(activeFilter.getValue2())
                    .values(activeFilter.getValues())
                    .build();
            }
            
            // Check if attribute has active sort
            String currentSort = null;
            for (QueryContext.SortSpec sort : context.getSorts()) {
                if (sort.getAttribute().equals(attrName)) {
                    currentSort = sort.getDirection().name();
                    break;
                }
            }
            ...
            QueryResult.QueryMetadata.AttributeMetadata attrMetadata = 
                QueryResult.QueryMetadata.AttributeMetadata.builder()
                    .name(attrName)
    ...
    private Map<String, QueryResult.QueryMetadata.FilterMetadata> buildAppliedFilters() {
        ...
        for (Map.Entry<String, QueryContext.Filter> entry : context.getFilters().entrySet()) {
            ...
            QueryResult.QueryMetadata.FilterMetadata filterMetadata = 
                QueryResult.QueryMetadata.FilterMetadata.builder()
    ...
    private Map<String, QueryResult.QueryMetadata.ParameterMetadata> buildParametersMetadata() {
        ...
        for (Map.Entry<String, ParamDef> entry : definition.getParams().entrySet()) {
            ...
            QueryResult.QueryMetadata.ParameterMetadata paramMetadata = 
                QueryResult.QueryMetadata.ParameterMetadata.builder()
                    .name(paramName)
                    .value(value)
