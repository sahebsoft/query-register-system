...
/**
 * Validates that all bind parameters in SQL queries and criteria are defined in the query definition.
...
public class BindParameterValidator {
        ...
        if (queryDef.getCriteria() != null) {
            for (Map.Entry<String, CriteriaDef> entry : queryDef.getCriteria().entrySet()) {
                ...
                if (criteria.getSql() != null) {
                    ...
                    // Also check if criteria declares its bind params
                    if (criteria.getBindParams() != null) {
                        // Validate that declared params match actual params in SQL
                        Set<String> declaredParams = criteria.getBindParams();
                        Set<String> undeclaredInCriteria = new HashSet<>(params);
                        undeclaredInCriteria.removeAll(declaredParams);
                        
                        if (!undeclaredInCriteria.isEmpty()) {
                            throw new IllegalStateException(String.format(
                                "Query '%s' criteria '%s' uses bind parameters that are not declared in bindParams: %s. " +
                                "SQL: %s",
                                queryName, criteria.getName(), undeclaredInCriteria, criteria.getSql()
                            ));
                        }
                    }
                }
    ...
    public static class ValidationResult {
        ...
        public Set<String> getUndefinedParams() { return undefinedParams; }
        public Set<String> getUnusedParams() { return unusedParams; }
    }
}